...
class TaDataFrame(pd.DataFrame):
    ...
    def parse(label, series):
        for indicator in Indicators:
            ...
            label_period = (
                int(label_parts[1])
                if str(label_parts[1]).isdigit()
                else str(label_parts[1])
            )
...
class BacktestingTaDataFrame(TaDataFrame):
    ...
    def apply_strategy(
        self, strategy: TradingStrategy, start: Optional[str] = None
    ) -> pd.DataFrame:
        # TODO: Add support for filtering out rejected and canceled orders
        funds_and_balance = {
            "residual_funds": self.initial_funds,
        ...
        order_contexts = applicable_portion.apply(
            lambda record: BacktestingTaDataFrame._apply_strategy_on_record(
                record, strategy, funds_and_balance, self.min_amount
            ),
            axis=1,
        )
        decisions = order_contexts.apply(
            lambda oc: oc.order.decision.name if oc.order else np.NaN
        )
        amounts = order_contexts.apply(lambda oc: oc.order.amount if oc.order else 0.0)
        statuses = order_contexts.apply(
            lambda oc: oc.order.status.name if oc.order else np.NaN
        )
        funds = order_contexts.apply(lambda oc: oc.funds)
        total_funds_over_time = order_contexts.apply(
            lambda oc: oc.total_funds_over_time
        )
        ...
        initial_buy_hold_balance = (
            total_funds_over_time.iloc[-1] / applicable_portion.iloc[0]["close"]
        )
        buy_hold = applicable_portion["close"].apply(
            lambda price: price * initial_buy_hold_balance
        )
    ...
    @staticmethod
    def _apply_strategy_on_record(
        record, strategy: TradingStrategy, funds_and_balance: dict, min_amount: float
...
class ExponentialMovingAverage(Indicator):
    ...
    def create(data: TaDataFrame, period):
        assert (
            type(period) == int
        ), "Only an integer number of periods is supported at the moment!"
